...
{
    [CustomEditor (typeof(StandardAssets_SunShafts))]
    class StandardAssets_SunShaftsEditor : Editor
    {
        ...
        public override void OnInspectorGUI () {
            ...
            if ((target as StandardAssets_SunShafts).GetComponent<Camera>())
                GUILayout.Label("Current camera mode: "+ (target as StandardAssets_SunShafts).GetComponent<Camera>().depthTextureMode, EditorStyles.miniBoldLabel);
		...
		*/
            ...
            if ((target as StandardAssets_SunShafts).sunTransform && (target as StandardAssets_SunShafts).GetComponent<Camera>()) {
                if (GUILayout.Button("Center on " + (target as StandardAssets_SunShafts).GetComponent<Camera>().name)) {
                    if (EditorUtility.DisplayDialog ("Move sun shafts source?", "The SunShafts caster named "+ (target as StandardAssets_SunShafts).sunTransform.name +"\n will be centered along "+(target as StandardAssets_SunShafts).GetComponent<Camera>().name+". Are you sure? ", "Please do", "Don't")) {
                        Ray ray = (target as StandardAssets_SunShafts).GetComponent<Camera>().ViewportPointToRay(new Vector3(0.5f,0.5f,0));
                        (target as StandardAssets_SunShafts).sunTransform.position = ray.origin + ray.direction * 500.0f;
                        (target as StandardAssets_SunShafts).sunTransform.LookAt ((target as StandardAssets_SunShafts).transform);
                    }
                }
            }
